...
logger = logging.getLogger("signac-flow-profiling")
...
def benchmark_project(project, keys=None):
    ...
    def run(key, func, repeat=3, number=10):
        if keys is None or key in keys:
            logger.info(f"Run '{key}'...")
            elapsed_times = []
            for r in range(repeat):
                # Setup steps that occur once per repeat
                start_time = time.perf_counter()
                flowproject = BenchmarkProject.get_project(
                    root=root, environment=TEST_ENV
                )
            ...
            data[key] = elapsed_times

    run(
        "print_status",
        lambda project: project.print_status(no_parallelize=True),
    )

    run(
        "run_operations",
        lambda project: project.run(),
    )

    run(
        "submit_operations",
        lambda project: project.submit(),
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Profile the runtime performance of signac-flow for basic workflow operations."
    )
    ...
    parser_report.add_argument(
        ...
        default="profiling.txt",
        nargs="?",
        help="The collection that contains the benchmark data (default='profiling.txt').",
    )
    parser_report.add_argument(
