...
{% block tasks %}
    ...
    {% if gpu_tasks %}
        ...
        {% endif %}
        {# GPU nodes have 9 NVIDIA K80s #}
        {% set nn_gpu = gpu_tasks|calc_num_nodes(9) %}
        {% set nn = nn_gpu %}
        {% else %}
        {% if 'gpu' in partition and not force %}
            {% raise "Requesting gpu partition, but no GPUs requested!" %}
        {% endif %}
        {% set nn = nn|default(cpu_tasks|calc_num_nodes(704), true) %}
      {% endif %}
      {% if 'gpu' in partition %}
      ...
      {% endif %}
      {% if partition == 'gpu' %}
  #SBATCH -N {{ nn|check_utilization(gpu_tasks, 9, threshold, 'GPU') }}
  ...
  #SBATCH --ntasks-per-node={{ (32, cpu_tasks)|min }}
      {% else %}
  {# This should cover bash #}
  #SBATCH -N {{ nn|check_utilization(cpu_tasks, 32, threshold, 'CPU') }}
  #SBATCH --ntasks-per-node={{ (704, cpu_tasks)|min }}
      {% endif %}
{% endblock tasks %}
{% block header %}
    {{ super() -}}
