...
{% set format_lable = ("%%-%ss" | format(label_length,)) %}

{% block body %}
# Overview:
Total # of jobs: {{'%s' |format(total_jobs)}}

{{ format_progress_bar | format('label','ratio')}}
{{ format_progress_bar | format('-'*label_length,'-'*bar_length)}}
{% for label in progress_sorted %}
{{ format_progress_bar | format(label[0], label[1] | draw_progressbar(total_jobs))}}
{% endfor %}

{% if detailed %}
# Detailed View:
{{format_job_id | format('job_id',)}} {{ format_operation_title | format('operation',)}} {{ '%s' | format('labels') }}
{{format_job_id | format('-'*id_length,)}} {{ format_operation | format('-'*(operation_length+4),)}} {{ '%s' | format('-'*total_label_length) }}
{% for job in jobs %}
