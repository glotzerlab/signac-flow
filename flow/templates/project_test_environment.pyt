...
@Project.operation(directives={"np": max(8, CPUS_PER_NODE)})
...
@Project.operation(directives={"nranks": max(8, CPUS_PER_NODE)})
def intra_node_mpi(job):
    ...
    size = comm.Get_size()
    if rank == 0:
        job.doc["intra_node_mpi"] = size == max(8, CPUS_PER_NODE))
