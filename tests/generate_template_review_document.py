...
DOC_DIR = os.path.join(os.path.dirname(__file__), "compiled_scripts")


def add_param_header(document, job):
    h = [f"{key}={val}" for key, val in job.sp.parameters.items()]
    h = "Parameters: " + ", ".join(h)
...
def process_job(document, job):
    ...
    if job.sp.parameters.get("bundle", False):
        ...
        for fn, h in name_map.items():
            if job.isfile(fn):
                document.add_heading(f"Operation: {h}", level=3)
                with open(job.fn(fn)) as fr:
                    document.add_paragraph(fr.read(), style="Code")
...
def main():
    ...
    with open("define_template_test_project.py") as file:
    ...
    try:
        ...
        print(
            "The templates project could not be found. Try running "
    ...
    for env in sorted(environments):
        ...
        p.add_run(f"{commit}", style="CodeChar")
        ...
        fn = os.path.join(DOC_DIR, f"{env_name}.docx")
        document.save(fn)
        print(f"Generated document '{fn}'.")
